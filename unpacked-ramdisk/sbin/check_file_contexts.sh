...
ctx_M=/file_contexts
ziptool unzip -p "$zip_path" META-INF/com/android/file_contexts.bin  > $new_ctx_bin
...
if [[ -s $new_ctx_bin ]]; then
    ...
    if [[ -s $ctx_M ]]; then
        ...
        con_diff=`toybox diff $new_ctx_bin $cur_ctx_bin`
    fi
    ...
    if [ x"$con_diff" = x ] ;then
        ...
        ziptool unzip -p "$zip_path" META-INF/com/android/file_contexts.bin > /file_contexts.bin
        restorecon -RF /data
    fi
fi
